...
resource "azurerm_network_security_group" "public" {
  ...
  # Only Application Gateway can reach the frontend application
  security_rule {
  ...
  # Bastion can SSH to frontend VMSS
  security_rule {
  ...
  # Block other VNet-originated inbound traffic.
  # Required allows above are evaluated first.
  security_rule {
...
resource "azurerm_network_security_group" "private" {
  ...
  # Frontend subnets can call backend application
  security_rule {
  ...
  # Bastion can SSH to backend VMSS
  security_rule {
    name                       = "AllowSSHFromBastion"
    priority                   = 120
    direction                  = "Inbound"
  ...
  # Reject other VNet inbound traffic
  security_rule {
    name                       = "DenyOtherVnetInbound"
    priority                   = 200
    direction                  = "Inbound"
  ...
  # Outbound Internet is routed through NAT Gateway
  security_rule {
...
resource "azurerm_network_security_group" "database" {
  ...
  # Only backend subnets can connect to PostgreSQL
  security_rule {
    name                       = "AllowBackendToPostgreSQL"
    priority                   = 100
    direction                  = "Inbound"
    access                     = "Allow"
    protocol                   = "Tcp"
...
# Associate NAT Gateway with frontend subnets
